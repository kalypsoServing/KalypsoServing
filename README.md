...
```mermaid
flowchart TB
    subgraph KalypsoProject["🏗️ KalypsoProject"]
        direction TB
        P[sample-project]
        subgraph Environments["Managed Namespaces"]
            direction LR
            DEV["🔧 dev-ns<br/>(Namespace)"]
            STAGE["🧪 stage-ns<br/>(Namespace)"]
            PROD["🚀 prod-ns<br/>(Namespace)"]
        end
        P --> DEV
        P --> STAGE
        P --> PROD
    ...
    subgraph KalypsoApplication["📦 KalypsoApplication"]
        direction TB
        A["recommendation-app"]
        A --> |projectRef| P
        A --> |storage| S3["☁️ S3 Credentials"]
    end

    subgraph KalypsoTritonServer["🤖 KalypsoTritonServer"]
        direction TB
        T["recommendation-v1"]
        T --> |applicationRef| A
        subgraph K8sResources["Managed K8s Resources"]
            direction LR
            DEPLOY["📋 Deployment<br/>(tritonserver)"]
            SVC["🌐 Service<br/>(http/grpc/metrics)"]
        end
        T --> DEPLOY
        T --> SVC
    end

    KalypsoProject -.-> KalypsoApplication
    KalypsoApplication -.-> KalypsoTritonServer

    style KalypsoProject fill:#1a365d,stroke:#2c5282,color:#fff
    style KalypsoApplication fill:#234e52,stroke:#285e61,color:#fff
    style KalypsoTritonServer fill:#553c9a,stroke:#6b46c1,color:#fff
    style Environments fill:#2d3748,stroke:#4a5568,color:#fff
    style K8sResources fill:#2d3748,stroke:#4a5568,color:#fff
```
